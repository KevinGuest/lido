...
export async function loadEllSwift(): Promise<EllSwiftApi> {
  ...
  // 1) createRequire from this file (works in Nest runtime + most CJS hosts)
  try {
    mod = requireFromHere('@scure/btc-signer/p2p.js');
  } catch (error) {
    errors.push(`createRequire: ${(error as Error).message}`);
  }

  // 2) Absolute path via package root (helps when package exports confuse Jest)
  if (!mod) {
    try {
      const path = require('path') as typeof import('path');
      const pkgDir = path.dirname(requireFromHere.resolve('@scure/btc-signer'));
      mod = requireFromHere(path.join(pkgDir, 'p2p.js'));
    } catch (error) {
      errors.push(`absolute: ${(error as Error).message}`);
    }
  }

  // 3) Dynamic import (Node ESM) — may fail under default Jest without --experimental-vm-modules
  if (!mod) {
    try {
      const importFn = new Function('specifier', 'return import(specifier)');
      mod = await importFn('@scure/btc-signer/p2p.js');
    } catch (error) {
      errors.push(`import: ${(error as Error).message}`);
      throw new Error(`Failed to load @scure/btc-signer EllSwift (${errors.join(' | ')})`);
    }
  }

  ellSwiftCache = mod.elligatorSwift as EllSwiftApi;
  if (!ellSwiftCache) {
    throw new Error('elligatorSwift export missing from @scure/btc-signer/p2p.js');
  }
